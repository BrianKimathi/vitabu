...
class _AudioBookState extends State<AudioBook> {
  ...
  addView(contentType, contentId, subContentId) async {
    if (Constant.userID != null) {
      await audioBookProvider.setView(contentType, contentId, subContentId);
    }
  }
  ...
  _scrollListener() {
    ...
    if (scrollController.offset >= scrollController.position.maxScrollExtent &&
        ...
        (audioBookProvider.currentPage ?? 0) < (audioBookProvider.totalPage ?? 0)) {
      audioBookProvider.setLoadMore(true);
      getApiData(audioBookProvider.currentPage ?? 0);
    }
  }

  getApiData(pageno) async {
    audioBookProvider.setLoading(true);
    await audioBookProvider.getProfile(Constant.userID ?? "");
    await audioBookProvider.getSectionList("1", Constant.userCategoryId, (pageno ?? 0) + 1);
  }

  @override
  void dispose() {
    scrollController.dispose();
    audioBookProvider.clearProvider();
    super.dispose();
  }
              ...
              children: [
                ...
                Expanded(
                  child: RefreshIndicator(
                    ...
                    color: Colors.white,
                    onRefresh: () async {
                      await audioBookProvider.getSectionList("1", Constant.userCategoryId, 0);
                    },
  ...
  Widget searchData() {
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        onTap: () {
          Navigator.of(context).push(PageRouteBuilder(
            pageBuilder: (context, animation, secondaryAnimation) => const Search(showType: "audiobook"),
            transitionDuration: const Duration(milliseconds: 200),
            transitionsBuilder: (context, animation, secondaryAnimation, child) {
              return ClipPath(
                clipper: CircularRevealClipper(progress: animation.value),
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: const Padding(
            ...
            child: Icon(Icons.headphones_rounded, color: colorPrimary, size: 22),
          ),
        ),
      ),
    );
  ...
  Widget _buildSectionData() {
    if (audioBookProvider.loading && !audioBookProvider.loadMore) {
      return sectionShimmer();
    } else {
      if (audioBookProvider.sectionListModel.status == 200 &&
          (audioBookProvider.sectionListData?.length ?? 0) > 0) {
        return SingleChildScrollView(
          controller: scrollController,
          physics: const AlwaysScrollableScrollPhysics(),
          child: Padding(
            padding: const EdgeInsets.only(bottom: 85),
            child: setSectionByType(audioBookProvider.sectionListData),
          ),
        );
      } else {
        return const NoData();
  ...
  Widget _buildAudioBanner(List<Datum>? sectionDataList, int sectionIndex, int? contentType) {
    return Column(
      children: [
        CarouselSlider.builder(
          ...
          options: CarouselOptions(
            onPageChanged: (index, reason) {
              audioBookProvider.setIndexChange(index: index);
            },
            height: 180,
        ...
        CarouselIndicator(
          ...
          index: audioBookProvider.currentIndex,
