...
const _abIndigo = Color(0xFF4E45B8);
const _abIndigoLight = Color(0xFFEEF0FF);
const _abTextDark = Color(0xFF0F172A);
const _abTextMedium = Color(0xFF6B7280);
const _abTextLight = Color(0xFF94A3B8);
const _abBorderLight = Color(0xFFF1F4F9);
const _abBorderHover = Color(0xFFD4D0F5);

class WebAudioBook extends StatefulWidget {
  const WebAudioBook({super.key});

  @override
  State<WebAudioBook> createState() => _WebAudioBookState();
}

class _WebAudioBookState extends State<WebAudioBook> {
  ...
  _scrollListener() {
    ...
    if (scrollController.offset >= scrollController.position.maxScrollExtent &&
      ...
      if ((categoryProvider.commanCurrentPage ?? 0) <
        ...
        _fetchbycommanapidata(categoryProvider.commanCurrentPage ?? 0, "1");
      }
    }
  }
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _abTextDark)),
              if (itemCount > 5)
                GestureDetector(
                  onTap: onToggle,
                  child: Icon(
                    isExpanded ? FontAwesomeIcons.angleUp : FontAwesomeIcons.angleDown,
                    size: 14, color: _abIndigo,
                  ),
                ),
            ],
          ),
          ...
          Expanded(
            child: GestureDetector(
              ...
              child: Text(
                ...
                style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: isSelected ? _abIndigo : _abTextDark),
              ),
            ),
          ),
        ],
  ...
  Widget _buildRightSideData() {
    ...
    return ResponsiveGridList(
      ...
      children: list.map((item) => _buildAudioCard(item)).toList(),
    );
  }

  // ── THE CARD — matches home page _ContentCard + headphones badge ──
  Widget _buildAudioCard(item) {
          ...
          child: GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: hovered ? _abBorderHover : _abBorderLight,
                  width: 1,
                ),
                boxShadow: [
                  BoxShadow(
                ...
                children: [
                  ...
                  Stack(
                    children: [
                      ...
                      // Headphones badge — distinguishes audio from books
                      Positioned(
                        bottom: 8, right: 8,
                        child: Container(
                          padding: const EdgeInsets.all(6),
                          decoration: BoxDecoration(
                            color: _abIndigo.withOpacity(0.88),
                            ...
                            child: Center(
                              child: Container(
                                ...
                                child: const Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.headphones_rounded, size: 16, color: Colors.white),
                                    SizedBox(width: 6),
                                    Text("Listen", style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                                  ],
                                ),
                              ),
                            ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if ((item.authorName ?? "").isNotEmpty)
                          Text(
                            item.authorName ?? "",
                            style: const TextStyle(color: _abTextLight, fontSize: 12, fontWeight: FontWeight.w400),
                            maxLines: 1, overflow: TextOverflow.ellipsis,
                          ),
                        ...
                        Row(
                          children: [
                            ...
                            Text(
                              ...
                              style: const TextStyle(color: _abTextDark, fontSize: 13, fontWeight: FontWeight.w600),
                            ),
                            const SizedBox(width: 4),
                            Text(
                            ...
                            if ((item.categoryName ?? "").isNotEmpty)
                              Flexible(
                                child: Text(
                                  item.categoryName ?? "",
                                  style: const TextStyle(color: _abTextLight, fontSize: 11),
                                  maxLines: 1, overflow: TextOverflow.ellipsis,
                                ),
                              ),
                          ],
  ...
  Widget _buildMobileViewData() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Utils.buildWebDetailsAppBar(context: context, isHome: true, title1: "audio_book", multilanguage: true),
          const SizedBox(height: 8),
          _buildFilterTabs(),
          const SizedBox(height: 12),
          _buildrightside(),
  ...
  Widget _titleBuild(int index, IconData icon, String title, VoidCallback onTap) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: selected ? _abIndigo : Colors.transparent,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: selected ? _abIndigo : const Color(0xFFE5E7EB), width: 1),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, size: 16, color: selected ? Colors.white : _abTextMedium),
            const SizedBox(width: 6),
            Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: selected ? Colors.white : _abTextMedium)),
          ],
        ),
      ),
    );
    ...
    return InteractiveContainer(
      child: (hovered) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isPrimary ? _abIndigo : Colors.transparent,
            borderRadius: BorderRadius.circular(8),
            border: isPrimary ? null : Border.all(color: const Color(0xFFE5E7EB)),
          ),
          child: Text(label, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isPrimary ? Colors.white : _abTextMedium)),
        ),
      ),
    );
  }
  ...
  Widget _buildFilterCategory({required List commanList, required String title}) {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: _abTextDark)),
          const SizedBox(height: 8),
            ...
            itemBuilder: (_, i) {
              ...
              return ListTile(
                ...
                title: GestureDetector(
                  onTap: toggle,
                  child: Text(name, maxLines: 2, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isSelected ? _abIndigo : _abTextDark)),
                ),
              );
            },
          ),
  ...
  Widget _buildRightSideShimmer() {
    return ResponsiveGridList(
      ...
      children: List.generate(6, (_) => Container(
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), border: Border.all(color: _abBorderLight, width: 1)),
