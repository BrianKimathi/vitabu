...
class LanguageWiseDataProvider extends ChangeNotifier {
  bool loading = false;
  bool loadMore = false;
  ...
  // Book List API
  Future<void> getSectionBook(type, languageId, pageno) async {
    loading = true;
    bookModel =
        await ApiService().languageByContentResponse(type, languageId, pageno);
  ...
  // Magazibe List API
  Future<void> getSectionMagazine(type, languageId, pageno) async {
    loading = true;
    magazineModel =
        await ApiService().languageByContentResponse(type, languageId, pageno);
  ...
  // Audio List API
  Future<void> getSectionAudio(type, languageId, pageno) async {
    loading = true;
    audioBookModel =
        await ApiService().languageByContentResponse(type, languageId, pageno);
...
/* Bookmark api Stared */
  Future<void> languageByContentResponse(index, contentType, contentId) async {
    if (contentType == "1") {
