...
class MylibraryProvider extends ChangeNotifier {
  ...
  // Book List API
  Future<void> getSectionBook(contentType, pageno) async {
    loading = true;
    bookModel = await ApiService().getBookMark(contentType, pageno);
  ...
  // Magazibe List API
  Future<void> getSectionMagazine(contentType, pageno) async {
    loading = true;
    magazineModel = await ApiService().getBookMark(contentType, pageno);
  ...
  // Audio List API
  Future<void> getSectionAudio(contentType, pageno) async {
    loading = true;
    audioBookModel = await ApiService().getBookMark(contentType, pageno);
...
/* Bookmark api Stared */
  Future<void> getBookMark(index, contentType, contentId) async {
    if (contentType == "1") {
