...
class AudioBookSectionController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $params['data'] = Content_Section::where('section_type', 1)->orderBy('sort_order', 'asc')->latest()->get();
            ...
            return view('admin.audio_book_section.index', $params);
        } catch (Exception $e) {
    ...
    public function store(Request $request)
    {
        try {
            ...
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_add_audio_book_section')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_add_audio_book_section')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data = Content_Section::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_edit_audio_book_section')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_edit_audio_book_section')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function show($id)
    {
        try {
            ...
            return response()->json(['status' => 200, 'success' => __('label.audio_book_section_delete')]);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
