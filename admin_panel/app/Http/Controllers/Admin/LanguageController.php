...
class LanguageController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $params['data'] = Language::orderby('sort_order', 'asc')->latest()->get();
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $delete = '<form onsubmit="return confirm(\'' . $language_delete . '\');" method="POST" action="' . route('admin.language.destroy', [$row->id]) . '">
                        ...
                        $btn .= '<a class="edit-delete-btn edit_language mr-4" data-toggle="modal" href="#EditModel" data-id="' . $row->id . '" data-name="' . $row->name . '" data-image="' . $row->image . '" title="' . __('label.edit') . '">';
    ...
    public function LanguageSortOrderSave(Request $request)
    {
        try {
