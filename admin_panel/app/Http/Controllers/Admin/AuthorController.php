...
class AuthorController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $author_delete = __('label.delete_author');

                        $delete = '<form onsubmit="return confirm(\'' . $author_delete . '\');" method="POST" action="' . route('admin.author.destroy', [$row->id]) . '">
                        ...
                        $btn .= '<a href="' . route('admin.author.edit', [$row->id]) . '" class="edit-delete-btn mr-4" title=' . __('label.edit') . '>';
            ...
            return view('admin.author.index', $params);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('admin.author.add', $params);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
    public function store(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'bank_holder_name' => 'required|min:2',
                'account_no' => 'required|min:2',
                'ifsc_code' => 'required|min:2',
            ]);
            if ($validator->fails()) {
            ...
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_add_author')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_add_author')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function edit($id)
    {
        try {
            ...
            if ($params['data'] != null) {
                ...
                return view('admin.author.edit', $params);
            } else {
    ...
    public function update($id, Request $request)
    {
        try {
            ...
            $data = User::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_edit_author')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_edit_author')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('admin.author.index')->with('success', __('label.author_delete'));
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
