...
class NovelsRequestController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                ...
                $data = Novel::with('author', 'category', 'language')->orderBy('id', 'desc')->where('status', 0)->latest()->get();
                ...
                return DataTables()::of($data)
                    ...
                    ->rawColumns(['action', 'chapter'])
                    ->make(true);
            }
            return view('admin.novels_request.index', $params);
        } catch (Exception $e) {
            ...
            if ($data && $status == 1) {
                ...
                if ($author) {
                    ...
                    if ($mail['status'] == 1 && $mail['send_notification'] == 1) {
                        $this->common->SaveNotification(1, 8, $author['id'], 2, 0, 0, 0, "", $author['device_type'], $author['device_token'], 1, $title);
                    }
                }
                return response()->json(['status' => 200, 'success' => __('label.novel_approved')]);
                ...
                if ($author) {
                    ...
                    if ($mail['status'] == 1 && $mail['send_notification'] == 1) {
                        $this->common->SaveNotification(1, 8, $author['id'], 2, 0, 0, 0, "", $author['device_type'], $author['device_token'], 0, $title);
                    }
                }
                return response()->json(['status' => 200, 'success' => __('label.novel_rejected')]);
            } else {
