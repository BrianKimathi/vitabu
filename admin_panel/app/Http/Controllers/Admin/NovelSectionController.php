...
class NovelSectionController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $params['data'] = Content_Section::where('section_type', 2)->orderBy('sort_order', 'asc')->latest()->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request['content_type'] == 2 || $request['content_type'] == 3) {
            ...
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_add_novel_section')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_add_novel_section')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data = Content_Section::updateOrCreate(['id' => $requestData['id']], $requestData);
            if (isset($data->id)) {
                return response()->json(['status' => 200, 'success' => __('label.success_edit_novel_section')]);
            } else {
                return response()->json(['status' => 400, 'errors' => __('label.error_edit_novel_section')]);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function show($id)
    {
        try {
            ...
            return response()->json(['status' => 200, 'success' => __('label.novel_section_delete')]);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
