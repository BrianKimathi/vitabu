    ...
    public function step0(Request $request)
    {
        ...
        if ($verflyDomain == 1) {
            ...
            try {
                ...
                // Skip installer when DB is already initialized.
                // If tbl_admin has at least one row, go directly to admin login.
                try {
                    $adminRow = DB::select('SELECT 1 FROM tbl_admin LIMIT 1');
                    if (!empty($adminRow)) {
                        return redirect()->route('admin.login');
                    }
                } catch (Exception $e) {
                    // Table may not exist / be empty yet. In that case, continue with purchase-code flow.
                }
