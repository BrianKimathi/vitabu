    ...
    <div class="body-content">
        ...
        <div class="border-bottom row mb-3">
            ...
            <div class="col-sm-1 d-flex justify-content-start mb-3">
                <button type="button" data-toggle="modal" data-target="#sortOrderModal" onclick="sortOrderBTN()" class="btn btn-default br-10" title="{{__('label.update_sort_order')}}">
        ...
        <div class="card custom-border-card mt-3">
            <h5 class="card-header">{{__('label.add_category')}}</h5>
            <div class="card-body">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group ml-3">
                                        ...
                                        <div class="avatar-upload">
                                            <div class="avatar-edit">
                                                <input type='file' name="image" id="imageUploadModel" accept=".png, .jpg, .jpeg, .webp" />
                                                <label for="imageUploadModel" title="{{__('label.upload_file')}}"></label>
                                            </div>
                                            <div class="avatar-preview">
                                                <img src="" id="imagePreviewModel">
                                            </div>
                                        </div>
        ...
        <div class="modal fade" id="sortOrderModal" tabindex="-1" data-backdrop="static" role="dialog" aria-labelledby="sortOrderModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title w-100 text-center" id="sortOrderModalLabel">{{__('label.category_sortorder_list')}}</h5>
                    ...
                    <div class="modal-footer justify-content-center">
                        <form enctype="multipart/form-data" id="save_category_sortorder">
                            @csrf
                            <input id="outputvalues" type="hidden" name="ids" value="" />
                            <div class="w-100 text-center">
                                <button type="button" class="btn btn-default mw-120" onclick="save_category_sortorder()">{{__('label.save')}}</button>
                            </div>
                        </form>
                    </div>
                </div>
...
<script>
        ...
        if (Demo_Mode == 1) {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'category', '{{ route("admin.category.index") }}');
                },
        ...
        if (Demo_Mode == 1) {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    get_responce_message(resp, 'edit_category', '{{ route("admin.category.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
    ...
    function change_status(id) {
        ...
        if (Demo_Mode == 1) {
            ...
            $("#dvloader").show();
            var url = `{{ route('admin.category.show', '') }}/${id}`;
    ...
    var category_data = JSON.parse('<?php echo json_encode($data); ?>');
    ...
    function sortOrderBTN() {
        ...
        for (var i = 0; i < category_data.length; i++) {
            htmlContent += '<div id="' + category_data[i].id + '" class="listitemClass mb-2 cursor" >' +
                '<p class="m-2">' + category_data[i].name + '</p>' +
                '</div>';
        }
    ...
    function save_category_sortorder() {

        var Demo_Mode = '<?php echo Demo_Mode(); ?>';
        if (Demo_Mode == 1) {

            $("#dvloader").show();
            var formData = new FormData($("#save_category_sortorder")[0]);
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.category.sortorder.save") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_category_sortorder', '{{ route("admin.category.index") }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
