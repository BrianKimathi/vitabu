    ...
    <div class="body-content">
        ...
        <div class="border-bottom row mb-3">
            ...
            <div class="col-md-2 d-flex align-items-center justify-content-end mb-4">
                <a href="{{route('admin.salesaudiobooks.create')}}" class="btn btn-default mw-120">{{__('label.add_sales_report')}}</a>
            </div>
        </div>
        ...
        <div class="page-search my-3">
            ...
            <div class="sorting mr-2 w-25">
                <select class="form-control" name="input_episode" id="input_episode">
                    <option value="0" selected>{{__('label.all_episodes')}}</option>
                </select>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#input_user, #input_author, #input_audio_book, #input_episode').change(function() {
            table.draw();
        });
        ...
        $('#datatable').on('click', '.download-invoice-btn', function() {
            ...
            tableBody.push([{
                ...
                {
                    text: '{{Currency_code()}}' + Math.max(data.price, 0),
                    alignment: 'center',
                },
            ]);
        ...
        $('#input_audio_book').change(function() {
            ...
            $('#input_episode').empty('');
            $('#input_episode').append('<option value="" selected>{{__("label.all_episodes")}}</option>');
            $.ajax({
                headers: {
                ...
                success: function(resp) {
                    for (i = 0; i < resp.result.length; i++) {
                        $('#input_episode').append('<option value="' + resp.result[i]['id'] + '">' + resp.result[i]['title'] + '</option>');
                    }
                },
