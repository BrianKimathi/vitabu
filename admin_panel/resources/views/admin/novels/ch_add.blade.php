            ...
            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('admin.novels.chapters.index', $novel_id) }}">{{__('label.chapters')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.add_chapter')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('admin.novels.chapters.index', $novel_id) }}" class="btn btn-default mw-120 mb-3">{{__('label.chapters')}}</a>
                </div>
            </div>
            ...
            <div class="card custom-border-card">
                <form id="add_chapter" enctype="multipart/form-data">
                    ...
                    <div class="border-top pt-3 text-right">
                        ...
                        <a href="{{route('admin.novels.chapters.index', $novel_id)}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
	...
	<script>
        ...
        function save_chapter() {
            ...
            if (Demo_Mode == 1) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("admin.novels.chapters.save") }}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'add_chapter', '{{ route("admin.novels.chapters.index", ["id" => $novel_id]) }}');
                    },
