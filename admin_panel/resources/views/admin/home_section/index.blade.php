...
@section('page_title', __('label.home_section'))
@section('tab_title', __('label.home_section'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{ __('label.home_section') }}</h1>

        <div class="border-bottom row mb-3">
            <div class="col-sm-11">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __('label.dashboard') }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __('label.home_section') }}</li>
                </ol>
            </div>
        ...
        <div class="card custom-border-card">
            <h5 class="card-header">{{ __('label.add_home_section') }}</h5>
            <div class="card-body">
                <form id="save_home_section" enctype="multipart/form-data">
                        ...
                        <div class="col-md-3 content_type_drop">
                            <div class="form-group">
                                ...
                                <select name="content_type" class="form-control" id="content_type">
                                    ...
                                    <option value="2">{{ __('label.novels') }}</option>
                    ...
                    <div class="border-top pt-3 text-right">
                        <button type="button" class="btn btn-default mw-120" onclick="save_home_section()">{{ __('label.save') }}</button>
        ...
        <div class="card custom-border-card mt-3">
            <div class="card-header d-flex justify-content-between">
                <h5>{{ __('label.edit_home_section') }}</h5>
                <div class="switch">
            ...
            <div class="card-body">
                ...
                <div class="border-top pt-3 text-right">
                    <button type="button" data-toggle="modal" data-target="#editSectionModal" onclick="edit_home_section('{{ $section['id'] }}')" class="btn btn-default mw-120">{{ __('label.update') }}</button>
                    <button type="button" onclick="delete_home_section('{{ $section['id'] }}')" class="btn btn-cancel mw-120 ml-2">{{ __('label.delete') }}</button>
                </div>
            </div>
        </div>
        @endforeach
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editSectionModalLabel">{{ __('label.edit_home_section') }}</h5>
                        <button type="button" class="close text-dark" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <form id="edit_home_section" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default mw-120" onclick="update_home_section()">{{ __('label.update') }}</button>
        ...
        <div class="modal fade" id="sortOrderModal" tabindex="-1" data-backdrop="static" role="dialog" aria-labelledby="sortOrderModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title w-100 text-center" id="sortOrderModalLabel">{{ __('label.home_section_sort_order_list') }}</h5>
                    ...
                    <div class="modal-footer justify-content-center">
                        <form id="save_home_section_sortorder" enctype="multipart/form-data">
                            <input id="outputvalues" type="hidden" name="ids" value="" />
                            <div class="w-100 text-center">
                                <button type="button" class="btn btn-default mw-120" onclick="save_home_section_sortorder()">{{ __('label.save') }}</button>
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            </div>
                        </form>
                    </div>
...
<script>
    ...
    // Save Section
    function save_home_section() {

        var Demo_Mode = '<?php echo Demo_Mode(); ?>';
        if (Demo_Mode == 1) {

            $("#dvloader").show();
            var formData = new FormData($("#save_home_section")[0]);
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.homesection.store') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_home_section', "{{ route('admin.homesection.index') }}");
                },
    ...
    function change_status(id) {
        ...
        if (Demo_Mode == 1) {
            ...
            $.ajax({
                ...
                type: "POST",
                url: "{{ route('admin.homesection.status') }}",
                data: {
                    id: id
                },
                success: function(resp) {
    ...
    function delete_home_section(id) {
        ...
        if (Demo_Mode == 1) {
            ...
            var result = confirm('{{__("label.delete_home_section")}}');
            if (result) {

                $("#dvloader").show();

                var url = "{{ route('admin.homesection.show', ':id') }}";
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, '', "{{ route('admin.homesection.index') }}");
                    },
                    error: function(XMLHttpRequest, textStatus, errorThrown) {
                        $("#dvloader").hide();
                        toastr.error(errorThrown, textStatus);
    ...
    // Update Section
    function edit_home_section(id) {

        let editUrl = "{{ route('admin.homesection.edit', ':id') }}".replace(':id', id);
        ...
        $.ajax({
            ...
            success: function(resp) {
                ...
                if (resp.data != null) {
                    ...
                    if (resp.data.content_type == 1 || resp.data.content_type == 2 || resp.data.content_type == 3) {
    ...
    function update_home_section() {

        var Demo_Mode = '<?php echo Demo_Mode(); ?>';
        if (Demo_Mode == 1) {

            $("#dvloader").show();
            var formData = new FormData($("#edit_home_section")[0]);

            var id = $('#edit_id').val();
            var url = "{{ route('admin.homesection.update', ':id') }}";
            url = url.replace(':id', id);
            ...
            $.ajax({
                ...
                success: function(resp) {
                    ...
                    get_responce_message(resp, 'edit_home_section', "{{ route('admin.homesection.index') }}");
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
    ...
    function save_home_section_sortorder() {

        var Demo_Mode = '<?php echo Demo_Mode(); ?>';
        if (Demo_Mode == 1) {

            $("#dvloader").show();
            var formData = new FormData($("#save_home_section_sortorder")[0]);
            $.ajax({
                type: 'POST',
                url: "{{ route('admin.homesection.sortable') }}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_home_section_sortorder', "{{ route('admin.homesection.index') }}");
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown, textStatus);
