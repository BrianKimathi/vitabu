...
@section('page_title', __('label.magazine_sales_report'))
@section('tab_title', __('label.magazine_sales_report'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{__('label.magazine_sales_report')}}</h1>

        <div class="border-bottom row mb-3">
            <div class="col-sm-10">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.magazine_sales_report')}}</li>
                </ol>
            </div>
            <div class="col-md-2 d-flex align-items-center justify-content-end mb-4">
                <a href="{{route('admin.salesmagazines.create')}}" class="btn btn-default mw-120">{{__('label.add_sales_report')}}</a>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        var table = $('#datatable').DataTable({
            ...dataTableDefaults,
            ajax: {
                url: "{{ route('admin.salesmagazines.index') }}",
                data: function(d) {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (data) {
                            let magazineTitle = data.title || '';
                            return `
                                    <div class="text-left">${magazineTitle}</div>
                                `;
                        } else {
                            return "-";
                        }
        ...
        $('#input_user, #input_author, #input_magazine').change(function() {
            table.draw();
        });
        ...
        $('#datatable').on('click', '.download-invoice-btn', function() {
            ...
            let tableBody = [
                ...
                [{
                        text: data.magazine['title'] + '(' + 'Magazine' + ')',
                        alignment: 'center'
                    },
                    {
            ...
            pdfMake.createPdf(docDefinition).download('{{__("label.magazine_sales_invoice")}}' + data.id + '.pdf');
        });
    });
</script>
@endsection