        ...
        <div class="border-bottom row mb-3">
            <div class="col-sm-10">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.salesmagazines.index') }}">{{__('label.magazine_sales_report')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.add_sales_report')}}</li>
                </ol>
            </div>
            <div class="col-sm-2 d-flex align-items-center justify-content-end">
                <a href="{{ route('admin.salesmagazines.index') }}" class="btn btn-default mw-120 mb-3">{{__('label.sales_report_list')}}</a>
            </div>
        </div>
            ...
            <div class="card-body py-0">
                <form id="save_sales_report" enctype="multipart/form-data">
                    ...
                    <div class="form-row">
                        <div class="col-md-12">
                            <div class="form-row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label>{{__('label.magazine')}}<span class="text-danger">*</span></label>
                                        <select class="form-control" name="magazine_id" id="magazine_id">
                                            <option value="">{{__('label.select_magazine')}}</option>
                    ...
                    <div class="border-top pt-3 text-right">
                        ...
                        <a href="{{route('admin.salesmagazines.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
...
<script>
    ...
    function save_sales_report() {
        ...
        if (Demo_Mode == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.salesmagazines.store") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_sales_report', '{{ route("admin.salesmagazines.index") }}');
                },
    ...
    $('#author_id').change(function() {
        ...
        $('#magazine_id').empty('');
        $('#magazine_id').append('<option value="" selected>{{__("label.select_magazine")}}</option>');
        $.ajax({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            type: 'POST',
            url: '{{ route("admin.salesmagazines.get_magazine") }}',
            data: {
                id: id
            },
            success: function(resp) {
                for (i = 0; i < resp.result.length; i++) {
                    $('#magazine_id').append('<option value="' + resp.result[i]['id'] + '">' + resp.result[i]['title'] + '</option>');
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                toastr.error(errorThrown, textStatus);
