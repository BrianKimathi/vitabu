    ...
    <div class="body-content">
        ...
        <div class="border-bottom row mb-3">
            ...
            <div class="col-sm-2 d-flex align-items-center justify-content-end">
                <a href="{{ route('admin.audiobooks.index') }}" class="btn btn-default mw-120 mb-3">{{__('label.audiobooks')}}</a>
            </div>
        </div>

        <!-- Search -->
        <form action="{{ route('admin.audiobooks.episodes.index', $audio_book_id)}}" method="GET">
        ...
        <div class="row">
            <div class="col-12 col-sm-6 col-md-4 col-xl-3">
                <a href="{{ route('admin.audiobooks.episodes.add', $audio_book_id) }}" class="add-video-btn">
                    ...
                    <div class="position-relative">
                        ...
                        <ul class="list-inline overlap-control">
                            <li class="list-inline-item">
                                <a class="btn" href="{{route('admin.audiobooks.episodes.edit', [$audio_book_id, $value->id])}}">
                                    <i class="fa-solid fa-pen-to-square fa-xl primary-color"></i>
                                </a>
                            </li>
                            <li class="list-inline-item">
                                <a class="btn" href="{{route('admin.audiobooks.episodes.delete', [$audio_book_id, $value->id])}}" onclick="return confirm('{{ __('label.delete_episode') }}')">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>{{__('label.showing')}} {{$data->firstItem()}} {{__('label.to')}} {{$data->lastItem()}} {{__('label.of_total')}} {{$data->total()}} {{__('label.entries')}}</div>
...
<script>
    ...
    function save_episode_sortorder() {
        ...
        if (Check_Admin == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.audiobooks.episodes.sortorder") }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'save_episode_sortorder', '{{ route("admin.audiobooks.episodes.index", $audio_book_id)}}');
                },
