...
@section('page_title', __('label.magazines_request'))
@section('tab_title', __('label.magazines_request'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{__('label.magazines_request')}}</h1>

        <div class="border-bottom row mb-3">
            <div class="col-sm-12">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.magazines_request')}}</li>
                </ol>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(function() {
            var table = $('#datatable').DataTable({
                ...dataTableDefaults,
                ajax: {
                    url: "{{ route('admin.magazines_request.index') }}",
                },
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
    ...
    function change_status(id, status) {
        ...
        if (Demo_Mode == 1) {
            ...
            $("#dvloader").show();
            var url = "{{route('admin.magazines_request.show', '')}}" + "/" + id;
            ...
            $.ajax({
                ...
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, '', '{{route("admin.magazines_request.index")}}');
                },
