...
@section('page_title', __('label.edit_magazines'))
@section('tab_title', __('label.edit_magazines'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{__('label.edit_magazines')}}</h1>

        <div class="border-bottom row mb-3">
            <div class="col-sm-10">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.magazines.index') }}">{{__('label.magazines')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_magazines')}}</li>
                </ol>
            </div>
            <div class="col-sm-2 d-flex align-items-center justify-content-end">
                <a href="{{ route('admin.magazines.index') }}" class="btn btn-default mw-120 mb-3">{{__('label.magazines_list')}}</a>
            </div>
        </div>

        <div class="card custom-border-card">
            <div class="card-body py-0">
                <form id="edit_magazines" enctype="multipart/form-data">
                        ...
                        <div class="col-md-9">
                            ...
                            <div class="form-row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <div class="d-block">
                                            <label>{{__('label.upload_magazines')}}<span class="text-danger">*</span></label>
                                            <div id="filelist2"></div>
                                            <div id="container2" class="position-relative">
                                                <div class="form-group">
                                                    <input type="file" id="uploadFile2" name="uploadFile2" class="form-control import-file p-2" accept=".pdf, .epub">
                                                </div>
                                                <input type="hidden" name="full_magazine" id="magazine_file_name" class="form-control">
                                            </div>
                                        </div>
                                        <input type="hidden" name="old_full_magazine" value="{{ $data['full_magazine'] }}">
                                        <a href="{{ $data['full_magazine'] }}" target="_blank" class="btn-link">{{ basename($data['full_magazine']) }}</a>
                                    </div>
                                </div>
                                <div class="col-md-2 mt-4">
                                    <div class="form-group mt-3">
                                        <a id="upload2" class="btn text-white bg-primary-color">{{__('label.upload_magazines')}}</a>
                                    </div>
                                </div>
                    ...
                    <div class="border-top pt-3 text-right">
                        <button type="button" class="btn btn-default mw-120" onclick="edit_magazines()">{{__('label.update')}}</button>
                        <a href="{{route('admin.magazines.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
...
<script>
    ...
    // Update Magazines
    function edit_magazines() {

        var Demo_Mode = '<?php echo Demo_Mode(); ?>';
        if (Demo_Mode == 1) {

            $("#dvloader").show();
            var formData = new FormData($("#edit_magazines")[0]);
            $.ajax({
                type: 'POST',
                url: '{{ route("admin.magazines.update", [$data->id]) }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'edit_magazines', '{{ route("admin.magazines.index") }}');
                },
