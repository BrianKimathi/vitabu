...
@section('page_title', __('label.magazines'))
@section('tab_title', __('label.magazines'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{__('label.magazines')}}</h1>

        <div class="border-bottom row mb-3">
            <div class="col-sm-10">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('author.dashboard') }}">{{__('label.dashboard')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.magazines')}}</li>
                </ol>
            </div>
            <div class="col-sm-2 d-flex align-items-center justify-content-end">
                <a href="{{ route('author.magazines.create') }}" class="btn btn-default mw-120 mb-3">{{__('label.add_magazines')}}</a>
            </div>
        </div>
        ...
        <div class="page-search mb-3">
            ...
            <div class="sorting mr-2 w-75">
                <select class="form-control" name="input_status" id="input_status">
                    <option value="" selected>{{__('label.all_status')}}</option>
                    <option value="0">
    ...
    $(document).ready(function() {
        ...
        $(function() {
            var table = $('#datatable').DataTable({
                ...dataTableDefaults,
                ajax: {
                    url: "{{ route('author.magazines.index') }}",
                    data: function(d) {
            ...
            $('#input_category, #input_language,#input_status,#input_access_type').change(function() {
                table.draw();
            });
