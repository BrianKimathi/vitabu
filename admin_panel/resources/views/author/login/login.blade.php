            ...
            <div class="mx-auto col-sm-11 col-md-9 col-xl-8 py-5">
                <div class="login-card-modern">
                    <div class="text-center mb-4">
                        <h2 class="font-weight-bold text-dark mb-1" style="font-size: 1.75rem;">Author & Publisher</h2>
                        <p class="text-muted" style="font-size: 0.9rem;">Sign in to manage your books and earnings</p>
                    </div>

                    <!-- Role Switcher Tabs (Admin vs Author) -->
                    <div class="role-switcher-container">
                        <a href="{{ route('admin.login') }}" class="role-tab-btn inactive">
                            <i class="fa-solid fa-user-gear"></i> Admin Login
                        </a>
                        <a href="{{ route('author.login') }}" class="role-tab-btn active">
                            <i class="fa-solid fa-pen-nib"></i> Author Login
                        </a>
                    </div>

                    @php
                    $emailValue = env('DEMO_MODE') == 'ON' ? 'author@author.com' : '';
                    $passwordValue = env('DEMO_MODE') == 'ON' ? 'author' : '';
                    @endphp
                    ...
                    <form id="login_form">
                        ...
                        <div class="text-right mb-4">
                            <a href="{{ route('author.forgot.password') }}" class="font-weight-600 text-primary" style="font-size: 0.85rem;">
                                Forgot password?
                            </a>
                        </div>
                    ...
                    <p class="text-center text-muted mb-0" style="font-size: 0.85rem;">
                        {{__('label.if_you_cannot_login_then')}} <a href="{{ env('APP_URL'). '/public/author/login' }}" target="_blank" class="font-weight-bold text-primary">{{__('label.click_here')}}</a>
                    </p>
                    @endif
                </div>
            </div>
    ...
    function save_login() {
        ...
        $.ajax({
            ...
            success: function(resp) {
                $("#dvloader").hide();
                get_responce_message(resp, 'login_form', '{{ route("author.dashboard") }}');
            },
