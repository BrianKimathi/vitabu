...
@section('page_title', __('label.add_chapter'))
@section('tab_title', __('label.add_chapter'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.add_chapter')}}</h1>
            ...
            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('author.novels.chapters.index', $novel_id) }}">{{__('label.chapters')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.add_chapter')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('author.novels.chapters.index', $novel_id) }}" class="btn btn-default mw-120 mb-3">{{__('label.chapters')}}</a>
                </div>
            </div>

            <div class="card custom-border-card">
                <form id="add_chapter" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="radio-group">
                                            <div class="custom-control custom-radio">
                                                <input type="radio" name="is_chapter_paid" id="is_chapter_paid_no" class="custom-control-input" value="0" checked>
                                                <label class="custom-control-label" for="is_chapter_paid_no">{{__('label.no')}}</label>
                                            </div>
                                            <div class="custom-control custom-radio">
                                                <input type="radio" name="is_chapter_paid" id="is_chapter_paid_yes" class="custom-control-input" value="1">
                            ...
                            <div class="form-row">
                                <div class="col-md-2">
                                    <div class="form-group">
                                        ...
                                        <select name="chapter_type" id="chapter_type" class="form-control">
                                            <option value="1">{{__('label.server_file')}}</option>
                                ...
                                <div class="col-md-6 external_url">
                                    <div class="form-group">
                                        ...
                                        <input type="url" name="chapter_url" class="form-control" placeholder="{{__('label.url_here')}}">
                                    </div>
                                </div>
                    ...
                    <div class="border-top pt-3 text-right">
                        <button type="button" class="btn btn-default mw-120" onclick="save_chapter()">{{__('label.save')}}</button>
                        <a href="{{route('author.novels.chapters.index', $novel_id)}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
	...
	<script>
        ...
        function save_chapter() {
            ...
            if (Demo_Mode == 1) {
                ...
                $("#dvloader").show();
                var formData = new FormData($("#add_chapter")[0]);
                $.ajax({
                    type: 'POST',
                    url: '{{ route("author.novels.chapters.save") }}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'add_chapter', '{{ route("author.novels.chapters.index", ["id" => $novel_id]) }}');
                    },
