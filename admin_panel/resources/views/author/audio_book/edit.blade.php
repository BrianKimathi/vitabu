...
@section('page_title', __('label.edit_audiobook'))
@section('tab_title', __('label.edit_audiobook'))
...
<div class="right-content">
    ...
    <div class="body-content">
        <!-- mobile title -->
        <h1 class="page-title-sm">{{__('label.edit_audiobook')}}</h1>
        ...
        <div class="border-bottom row mb-3">
            <div class="col-sm-10">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_audiobook')}}</li>
                </ol>
            </div>
            <div class="col-sm-2 d-flex align-items-center justify-content-end">
                <a href="{{ route('author.audiobooks.index') }}" class="btn btn-default mw-120 mb-3">{{__('label.audiobooks_list')}}</a>
            </div>
        </div>

        <div class="card custom-border-card">
            <div class="card-body py-0">
                <form id="edit_audiobook" enctype="multipart/form-data">
                                    ...
                                    <div class="form-group">
                                        <div class="d-block">
                                            ...
                                            <div id="container3" class="position-relative">
                                                <div class="form-group">
                                                    <input type="file" id="uploadFile3" name="uploadFile3" class="form-control import-file p-2" accept=".pdf, .epub">
                                                </div>
                                                <input type="hidden" name="full_audio" id="full_audio_file_name" class="form-control">
                                            </div>
                                        </div>
                                        <input type="hidden" name="old_full_audio" value="{{ $data['full_audio'] }}">
                                        <a href="{{ $data['full_audio'] }}" target="_blank" class="btn-link">{{ basename($data['full_audio']) }}</a>
                                    </div>
                                </div>
                    ...
                    <div class="border-top pt-3 text-right">
                        <button type="button" class="btn btn-default mw-120" onclick="edit_audiobook()">{{__('label.update')}}</button>
                        <a href="{{route('author.audiobooks.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
...
<script>
    ...
    function edit_audiobook() {
        ...
        if (Demo_Mode == 1) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("author.audiobooks.update", [$data->id]) }}',
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
                success: function(resp) {
                    $("#dvloader").hide();
                    get_responce_message(resp, 'edit_audiobook', '{{ route("author.audiobooks.index") }}');
                },
