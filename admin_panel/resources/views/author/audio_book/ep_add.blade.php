            ...
            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('author.audiobooks.episodes.index', $audio_book_id) }}">{{__('label.episodes')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.add_episode')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('author.audiobooks.episodes.index', $audio_book_id) }}" class="btn btn-default mw-120 mb-3">{{__('label.episodes')}}</a>
                </div>
            </div>
            ...
            <div class="card custom-border-card">
                <form id="add_episode" enctype="multipart/form-data">
                    ...
                    <div class="border-top pt-3 text-right">
                        ...
                        <a href="{{route('author.audiobooks.episodes.index', $audio_book_id)}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
	...
	<script>
        ...
        function save_episode() {
            ...
            if (Demo_Mode == 1) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("author.audiobooks.episodes.save") }}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'add_episode', '{{ route("author.audiobooks.episodes.index", ["id" => $audio_book_id]) }}');
                    },
